...
from .child_assent_model_wrapper import PreFlourishChildAssentModelWrapper as AssentModelWrapper
...
class PreFlourishSubjectConsentModelWrapper(ModelWrapper):
    ...
    def child_assents_exist(self):
        subject_identifiers = self.children_consent_objs.values_list('subject_identifier', flat=True)
        
        assents_exist = Assent.objects.filter(subject_identifier__in=subject_identifiers).exists()
    ...
    def child_assents(self):
        ...
        children_consents = self.children_consent_objs.filter(child_dob__lte = least_expected_dob)
